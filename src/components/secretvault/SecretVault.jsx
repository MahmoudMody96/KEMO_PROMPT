    ...
    return (
        <div
            ...
            onMouseEnter={(e) => {
                ...
                e.currentTarget.style.borderColor = `color-mix(in srgb, ${category.color} 20%, transparent)`;
                e.currentTarget.style.boxShadow = `0 8px 30px color-mix(in srgb, ${category.color} 8%, transparent)`;
            }}
            onMouseLeave={(e) => {
        ...
        >
            ...
            <div className="absolute top-0 left-0 right-0 h-px opacity-0 group-hover:opacity-100 transition-opacity"
                style={{ background: `linear-gradient(to right, transparent, color-mix(in srgb, ${category.color} 40%, transparent), transparent)` }} />
            ...
            <div className={`flex items-start justify-between mb-3 ${isArabic ? 'flex-row-reverse' : ''}`}>
                <div className={`flex-1 ${isArabic ? 'text-right' : ''}`}>
                    <div className={`flex items-center gap-2 mb-1 ${isArabic ? 'flex-row-reverse' : ''}`}>
                        ...
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                            style={{ background: `color-mix(in srgb, ${category.color} 12%, transparent)`, color: category.color }}>
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className="w-3 h-3" fill={i < prompt.impact ? 'var(--warn-fg)' : 'transparent'}
                            style={{ color: i < prompt.impact ? 'var(--warn-fg)' : 'var(--star-empty)' }} />
                    ))}
                </div>
            </div>

            {/* Use Case */}
            <p className={`text-xs text-muted leading-relaxed mb-4 ${isArabic ? 'text-right' : ''}`}>
                {isArabic ? prompt.useCase.ar : prompt.useCase.en}
            </p>

            {/* Preview snippet */}
            <div className="rounded-lg p-2.5 mb-3" style={{ background: 'var(--overlay-4)' }}>
                <p className="text-[10px] text-muted font-mono line-clamp-3 leading-relaxed" dir="ltr">
            ...
            <div className={`flex items-center gap-2 ${isArabic ? 'flex-row-reverse' : ''}`}>
                <button
                    ...
                    style={{
                        background: copied ? 'rgba(34,197,94,0.15)' : `color-mix(in srgb, ${category.color} 8%, transparent)`,
                        color: copied ? 'var(--success-fg)' : category.color,
                        border: `1px solid ${copied ? 'rgba(34,197,94,0.2)' : `color-mix(in srgb, ${category.color} 15%, transparent)`}`,
                    }}
                >
...
const PreviewModal = ({ prompt, category, isArabic, onClose }) => {
    ...
    const handleCopy = () => copy(prompt.prompt);

    // Replaces a bare Escape listener: this also traps Tab, locks background
    // scroll and restores focus to the card that opened the preview.
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4"
            ...
            <div
                ...
                role="dialog"
                aria-modal="true"
                aria-label={isArabic ? 'معاينة البرومبت' : 'Prompt preview'}
                className="relative w-full max-w-3xl max-h-[85vh] rounded-2xl overflow-hidden flex flex-col"
                style={{
                    background: 'var(--modal-bg)',
                    border: `1px solid color-mix(in srgb, ${category.color} 19%, transparent)`,
                    boxShadow: `var(--dropdown-shadow), 0 0 40px color-mix(in srgb, ${category.color} 6%, transparent)`,
                }}
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-white/5"
                    style={{ background: `color-mix(in srgb, ${category.color} 3%, transparent)` }}>
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={handleCopy}
                            ...
                            style={{
                                background: copied ? 'rgba(34,197,94,0.2)' : `linear-gradient(135deg, ${category.color}, color-mix(in srgb, ${category.color} 80%, transparent))`,
                                color: copied ? 'var(--success-fg)' : '#fff',
                            }}>
                ...
                <div className={`px-5 py-3 flex items-center gap-2 ${isArabic ? 'flex-row-reverse' : ''}`}
                    ...
                    <Sparkles className="w-3.5 h-3.5 text-[var(--warn-fg)] flex-shrink-0" />
                    <p className="text-xs text-text2">
                        <span className="font-bold text-text2">{isArabic ? 'متى تستخدمه: ' : 'Use case: '}</span>
                        {isArabic ? prompt.useCase.ar : prompt.useCase.en}
                    </p>
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    <pre className="text-sm text-text2 leading-relaxed whitespace-pre-wrap font-mono"
...
const SecretVault = () => {
    ...
    return (
        <div className="w-full pb-8" dir={isRTL ? 'rtl' : 'ltr'}>
            {/* Hero */}
            <div className="on-dark relative rounded-2xl overflow-hidden mb-6" style={{ minHeight: 160 }}>
                ...
                <div className="relative z-10 p-8 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-4"
                        ...
                        <Lock className="w-3.5 h-3.5 text-[var(--warn-fg)]" />
                        <span className="text-xs font-bold text-[var(--warn-fg)]">{isArabic ? 'حصري' : 'EXCLUSIVE'}</span>
                    </div>
                    ...
                    <p className="text-sm text-text2 max-w-lg mx-auto">
                        {isArabic
                    ...
                    <div className="flex items-center justify-center gap-4 mt-3 text-xs text-muted">
                        ...
                        <span className="w-1 h-1 rounded-full bg-bg2" />
            ...
            <div className={`flex items-center gap-3 mb-5 flex-wrap ${isArabic ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex-1 min-w-[200px] relative ${isArabic ? 'text-right' : ''}`}>
                    <Search className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 text-muted ${isArabic ? 'right-3' : 'left-3'}`} />
                    <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder={isArabic ? 'ابحث في البرومبتات...' : 'Search prompts...'}
                        className="w-full h-10 rounded-xl text-sm text-text2 placeholder-zinc-600"
                        style={{
            ...
            <div className={`flex items-center gap-2 mb-6 overflow-x-auto pb-2 ${isArabic ? 'flex-row-reverse' : ''}`}
                ...
                <button
                    ...
                    style={{
                        ...
                        color: activeCategory === 'all' ? 'var(--brand-fg)' : 'var(--text-muted)',
                    }}
                >

                        ...
                        <div className="relative p-3.5 md:p-5">
                            ...
                            {(isSuggesting || suggestedIdeas.length > 0) && (<>
                                ...
                                <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none">
                                    <div className="overflow-hidden rounded-2xl pointer-events-auto" style={{ animation: 'fadeIn 0.2s ease-out', width: 'min(90vw, 520px)', background: 'var(--modal-bg)', backdropFilter: 'blur(20px)', border: '1px solid rgba(245,158,11,0.2)', boxShadow: 'var(--dropdown-shadow)' }}>
                                                    ...
                                                    <button key={idx} onClick={() => handlePickIdea(idea, idx)} className="w-full text-start p-3.5 rounded-xl border transition-all duration-200 group cursor-pointer" dir="auto" style={{ animation: `slideUp 0.3s ease-out ${idx * 0.08}s both`, background: selectedIdeaIndex === idx ? 'rgba(245,158,11,0.1)' : 'var(--overlay-3)', borderColor: selectedIdeaIndex === idx ? 'rgba(245,158,11,0.3)' : 'var(--border-color)' }}>
                                                        <div className={`flex items-start gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <p className="text-[13px] font-medium leading-relaxed text-white/90 group-hover:text-white">{idea.title}</p>
                                                                {(!generatorInputs?.characters?.primary || generatorInputs.characters.primary.toLowerCase() === 'auto') && (
                                                                    <span className={`inline-block mt-1 text-[10px] px-1.5 py-0.5 rounded-full border font-medium ${idx === 0 ? 'bg-blue-500/15 text-blue-300 border-blue-500/25' : idx === 1 ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/25' : 'bg-purple-500/15 text-purple-300 border-purple-500/25'}`}>{idx === 0 ? (language === 'ar' ? '👤 إنسان' : '👤 Human') : idx === 1 ? (language === 'ar' ? '🐱 حيوان' : '🐱 Animal') : (language === 'ar' ? '🎲 عشوائي' : '🎲 Random')}</span>
                                                                )}
                                                            </div>
                                                        </div>
                    ...
                    <div className="relative group/section rounded-2xl overflow-hidden" style={{ background: 'rgba(6,182,212,0.03)', border: '1px solid rgba(6,182,212,0.12)' }}>
                        ...
                        <div className="relative p-3.5 md:p-5">
                            <div className={`flex items-center gap-3 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div><h3 className="text-sm font-bold text-text1">{safeT('productionSettings', 'Production Settings')}</h3><p className="text-[10px] text-zinc-500">{language === 'ar' ? 'نسبة العرض والمدة والمشاهد' : 'Aspect ratio, duration & scenes'}</p></div>
                            </div>

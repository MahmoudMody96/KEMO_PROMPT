...
═══ 🗣️ Natural Speech Injection (إلزامي — الحوار لازم يبان بشري) ═══
الحوار لازم يبان كأنه شخص حقيقي بيتكلم — مش AI:

1. **Filler Words**: أضف 1-2 filler word في كل 3 جمل باللهجة الصح:
   → ${dialectDNA.slangWords.slice(0, 5).map(s => s.split(' (')[0]).join(' | ')}
   → مثال: "${(dialectDNA.exampleDialogue || [''])[0].substring(0, 50)}..."

2. **Sentence Rhythm**: نوّع طول الجمل إلزامياً:
   → جملة قصيرة (3-5 كلمات) → متوسطة (8-12) → طويلة (15-20) → قصيرة
   → ممنوع 3 جمل ورا بعض بنفس الطول!
   → مثال صح ✅: "لأ. | مش كده خالص يعني، | أنا كنت فاكر إن الحكاية بسيطة بس اتضح إنها أكبر بكتير مما تخيلت. | فاهم؟"
   → مثال غلط ❌: "أنا مش فاهم الحكاية. | أنا مش عارف إيه الحل. | أنا مش قادر أفكر."

3. **Emotional Breath**: كل 2-3 جمل، أضف لحظة تنفس عاطفي:
   → وقفة (...)، أو تنهيدة، أو ضحكة، أو صمت معبّر
   → مثال: "يعني... (وقفة) أنا مش عارف, والله مش عارف أقول إيه"

${numCharacters > 1 ? `4. **Interruption Pattern**: في الحوار متعدد الشخصيات:
   → شخصية تقاطع التانية (—)
   → شخصية تكمّل جملة التانية
   → شخصية ترد بكلمة واحدة بس ("أيوه" / "لأ" / "يا نهار!")
   → ممنوع كل الشخصيات تتكلم بنفس الطول والإيقاع!` : ''}

═══ 🎭 Emotional Beats — خريطة المشاعر (إلزامي) ═══
كل مشهد ليه **شعور رئيسي** مختلف — لازم يكون واضح في الحوار والبصريات:
${Array.from({ length: numScenes }, (_, i) => {
    const beats = ['😐 Neutral — تأسيس وتعريف', '😲 Curiosity — فضول وتشويق', '😬 Tension — توتر وضغط', '😱 Climax — ذروة ومفاجأة', '😂 أو 😢 Release — تفريغ عاطفي', '💡 Revelation — إدراك وحكمة'];
    const beatIndex = Math.min(Math.floor(i * beats.length / numScenes), beats.length - 1);
    return `مشهد ${i + 1}: ${beats[beatIndex]}`;
  }).join('\n')}
⚠️ ممنوع مشهدين ورا بعض بنفس الشعور! لازم يكون فيه تصاعد وتنويع.

═══ الحوار — قواعد TTS ═══
• جمل متدفقة 8-15 كلمة — مش جمل مقطّعة — ونوّع الطول!
• فاصلة (,) للوقفات الطبيعية — بدل !!!
• نقط (...) للتردد والتفكير — بتدي حياة للحوار
